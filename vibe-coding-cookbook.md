...
> **成果**：一份技術選型**有依據**、任務拆解清楚的 `plan.md`　｜　**時間**：約 1–2 小時　｜　**難度**：中

規格回答了 WHAT，這個環節要回答 **HOW**。它就像 **TPM（技術專案經理）把規格落成實際開發計畫**：要用什麼技術、架構怎麼搭、工作怎麼拆成一步步的任務。

但有一個鐵則：**技術選型不能靠 AI 憑記憶決定**。AI 很會講出「聽起來合理」的方案，卻可能過時或根本不存在。所以這個環節**一定要用 Context7 + 並行研究，把選定方案的最佳實踐查清楚**，確保計畫的每個技術決定都站得住腳。

#### 你需要先有

- [ ] 環節 2 定稿的 `spec.md`。
- [ ] 已安裝並啟用 `writing-plans`、`Context7`、`dispatching-parallel-agents`。

#### 這個環節怎麼走

我們採「**研究在前**」：先把選定方案的最佳實踐查清楚、驗證效度，再讓計畫帶著研究結論落地。

```mermaid
flowchart TD
    A["spec.md 定稿（WHAT）"] --> B["先研究選定方案的最佳實踐<br/>Context7 + dispatching-parallel-agents"]
    B --> B1["正確 API 用法<br/>Context7 查官方文件"]
    B --> B2["重要參數 / 常見坑<br/>並行多 agent"]
    B1 & B2 --> C["writing-plans 產出 plan.md<br/>HOW：技術選型 + 架構 + 任務拆解"]
    C --> D{"你審閱<br/>選型有依據嗎?任務夠小嗎?"}
    D -->|要改| C
    D -->|確認| E["plan.md 定稿 → 交給環節 4"]
```

#### 先分清楚：這裡的 Context7 研究，和環節 1 不一樣

兩個環節都用 Context7，但**問的問題完全不同**——搞混會白做工：

| | 環節 1 · 需求釐清 | 環節 3 · 實作計劃 |
|---|---|---|
| 你在哪個階段 | 方案**還沒選定** | 方案**已經選定** |
| Context7 研究什麼 | 「針對這個需求，**大家通常選哪些解法**？」 | 「選定的這個方案，**最佳實踐怎麼做**？」 |
| 重點 | 探索並**比較幾個選項** | 鎖定一個方案的**正確用法、參數、踩坑** |
| 法規 AI 範例 | 「法規問答要**自建 RAG 還是用託管服務**？」比較選項 | 已選 OpenAI file search →「它的 **chunking 參數**怎麼設、Responses API 正確用法」 |

> {{icon:lightbulb}} **一句話記住**：環節 1 是「**選哪條路**」，環節 3 是「**這條路怎麼走得最好**」。

#### 步驟

**步驟 1 · 拿 spec.md 去研究技術社群的主流解法與最佳實踐**

直接請 AI 詳讀 superpowers 已寫好的 `spec.md`，針對規格要實作的每個部分，用 Context7 找技術社群的主流解法與最佳實踐——並**務必用 `dispatching-parallel-agents` 盡可能多派 agent 並行研究**，追求資訊覆蓋，最後把整理好的資訊向你說明。

> {{icon:clipboard}} **複製提示詞**
> ```text
> 請先詳細研究我們的 spec.md（superpowers 已寫好的規格）。
> 接著針對規格裡要實作的每個部分，使用 Context7 MCP 尋找技術社群的「主流解法與最佳實踐」。
> 請務必使用 dispatching-parallel-agents skill，盡可能派遣多個 agent 從各方面並行研究，
> 以追求資訊覆蓋（不用擔心派太多）。
> 研究完，把整理好的資訊向我說明。
> ```

> {{icon:circle-check}} **以法規問答 AI 為例**，並行 agent 可同時研究多個面向：file search 的 vector store 與 Responses API 正確用法、chunking 參數（`max_chunk_size_tokens`、`chunk_overlap_tokens`）對法規長文怎麼設、語料前處理、引用出處的解析方式…最後彙整成「我們該照做的最佳實踐」。

**步驟 2 · 確認最終方案 → 自動寫成 plan.md，然後審閱**

AI 把研究彙整給你後，你**確認最終的解決方案細節**。一旦確認，`writing-plans` 會**直接把它寫成 `plan.md`**——你不需要再額外下指令。

plan.md 出來後，一條一條審閱，重點檢查兩件事：**每個技術選型有沒有附依據？任務是不是夠小、每一步都能單獨驗證？**

#### 完成檢查（Checkpoint）

- [ ] `plan.md` 存在（HOW：技術選型 + 架構 + 任務拆解）。
- [ ] **每個關鍵技術選型都有依據**（來自 Context7 / 研究，不是憑空）。
- [ ] 任務**拆成小步**，每一步都能單獨驗證。
- [ ] 你**看得懂、而且同意**。
...
| 症狀 | 可能原因 | 怎麼辦 |
|---|---|---|
| 技術選型沒附理由 | AI 憑記憶選的 | 要它「用 Context7 查官方文件佐證每個選型」 |
| 任務太大、一步到位 | 沒拆解 | 要求「拆成可獨立驗證的小步，標出相依」 |
| Context7 查不到資料 | 套件太冷門 / 名稱不對 | 先 resolve 正確的 library id，或用 Web Search 補 |

> {{icon:arrow-right}} **下一步**：環節 4 · 正式開發——照著 plan.md，用 TDD 一步步把功能做出來。

---

### 環節 4 · 正式開發

> **成果**：plan.md 的任務逐一完成、**有測試覆蓋**的可運作程式　｜　**時間**：視專案，數小時起　｜　**難度**：中高

這是把計畫變成程式的環節，也是最多人「放手讓 AI 亂寫」的地方。重點不是「叫 AI 一次全部生出來」，而是用一套**可重複的開發循環**，讓每個任務都被測試驗證、被你理解、被審查。

範例會穿插示範（補 RAG 核心、加 Postgres…），但**你真正要記住的是「循環」本身，不是範例的技術細節**。

#### 你需要先有

- [ ] 環節 3 定稿的 `plan.md`（任務拆解）。
- [ ] 已啟用 `TDD`、`Context7`、`systematic-debugging`、`karpathy-guidelines`、`code review`、`dispatching-parallel-agents`。

#### 核心：每個任務都走「TDD 開發循環」

不管任務是大是小，都走同一個循環——這就是整個環節的主軸：

```mermaid
flowchart LR
    A["plan.md 任務"] --> R["紅 Red<br/>先寫會失敗的測試<br/>定義「成功長怎樣」"]
    R --> G["綠 Green<br/>寫最小實作讓測試通過<br/>不會的 API→Context7<br/>卡關→systematic-debugging"]
    G --> V["審 Review<br/>code review + karpathy 最小改動"]
    V --> Q{"還有任務?"}
    Q -->|有| R
    Q -->|沒有| E["核心功能完成 → 環節 5"]
```

> {{icon:lightbulb}} **為什麼要「先寫測試」？** 這呼應 Part 2 的 Karpathy「**目標驅動**」原則——先定義「成功」，AI 才能跑到達標。所以不要說「修這個 bug」，而要說「**先寫一個能重現 bug 的測試，再讓它通過**」。

| 循環三步 | 做什麼 | 用什麼 |
|---|---|---|
| **紅 Red** | 寫一個會失敗的測試，講清楚「做完要通過什麼」 | TDD |
| **綠 Green** | 寫**剛好**讓測試通過的最小實作 | Context7（查 API）、systematic-debugging（卡關） |
| **審 Review** | 檢查 bug、過度設計、改動是否對得上需求 | code review、karpathy-guidelines |

#### 步驟（其實就是反覆套用上面的循環）

**步驟 1 · 啟動：請 AI 照 plan.md 用 TDD 一個任務一個任務做**

> {{icon:clipboard}} **複製提示詞**
> ```text
> 請照著 plan.md，一個任務一個任務地開發，每個任務都用 TDD：
> 1. 先寫一個會失敗的測試，定義「這個任務做完會通過什麼」
> 2. 再寫剛好讓測試通過的最小實作（不要多做沒被要求的功能）
> 3. 過程中不確定某個 API / 套件用法，先用 Context7 查官方文件再寫
> 做完一個任務先停下來讓我看，確認後再做下一個。
> ```

> {{icon:circle-check}} **範例：把循環套在「補 RAG 核心」上**（法規問答 AI）
> starter 裡已經有**失敗的測試**。開發 `parse_response` 時：① 先看那個失敗測試、反推它期待的輸入輸出（**紅**）→ ② 用 Context7 查 file search 的回應結構，寫最小實作讓測試變綠（**綠**）→ ③ code review（**審**）。換成 `answer_question`、加 Postgres、做前端……**同一個循環照套**。

**步驟 2 · 卡關時：用 systematic-debugging 先重現再找根因**

> {{icon:clipboard}} **複製提示詞**
> ```text
> 這裡卡住了 / 測試一直失敗。請用 systematic-debugging skill：
> 先寫一個能穩定重現問題的最小案例，找出根因，再修。
> 不要只是加 timeout 或亂試——先讓我看到根因是什麼。
> ```

> {{icon:triangle-alert}} **注意**：AI 最愛用「加個等待」「再試一次」蒙混過去。不要接受——**沒講出根因的修法，等於沒修**。

**步驟 3 · 獨立任務並行：用 dispatching-parallel-agents 加速**

> {{icon:clipboard}} **複製提示詞**
> ```text
> plan.md 裡這幾個任務彼此獨立：<列出任務，例如：前端美化、Postgres 接入>。
> 請用 dispatching-parallel-agents skill 同時開發它們，最後把結果整合給我。
> ```

> {{icon:triangle-alert}} **注意**：只並行**會改到不同檔案、互不影響**的任務。會動到同一個檔案的任務並行，只會打架。

**步驟 4 · 每段完成：用 code review 把關**

> {{icon:clipboard}} **複製提示詞**
> ```text
> 這個功能做完了。請用 code review 檢查：有沒有 bug、有沒有過度設計、
> 每個改動是否都對得上需求（karpathy 最小改動）。
> 把問題列給我，我們再決定怎麼改。
> ```
...
- [ ] 每個功能都有**測試、且測試通過（綠）**。
- [ ] 你**讀得懂每一段**程式在做什麼。
- [ ] 碰到的**紅線**（安全 / 資料 / 部署）程式碼，已經停下來搞懂（見 Part 2）。

#### 疑難排解

| 症狀 | 可能原因 | 怎麼辦 |
|---|---|---|
| AI 想一次寫完所有東西 | 沒照 TDD 小步 | 要它「一次一個任務、先寫測試、做完停下來」 |
| 測試「為了過而過」 | AI 改測試去遷就實作 | 要求測試反映真實需求，**不准為了變綠而改測試的期望值** |
| AI 只加 timeout 蒙混 | 沒找根因 | 用 systematic-debugging，要求先重現、先講根因 |
| 改一個地方、壞一整片 | 改動範圍太大 | 用 karpathy：要它自審 diff、只動該動的程式碼 |

> {{icon:arrow-right}} **下一步**：環節 5 · 測試——讓 AI 真的開瀏覽器，把使用者流程從頭到尾跑一遍。
