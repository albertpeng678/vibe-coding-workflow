...
2. 依信心分級決定動作，二選一：
   ...
   B. 若為 Medium 或 High：
      → 不觸發提問，直接進入步驟 3 的收斂確認；不要用虛擬多 Agent 共識會議這種
        包裝手法假裝有嚴謹討論過程。

3. 主產品定位收斂確認（正式產出簡報前的強制檢查點——這是一個硬性的對話輪次邊界，
   不是流程清單裡的一個段落）：不論步驟 2 是否觸發提問，在開始草擬簡報大綱與講稿
   之前，必須先輸出一份精簡的「主產品定位摘要」，然後立即結束本次回覆，彙整
   全部欄位目前確定/推論出的最終結果（含剛回答的 Low 欄位、以及 Medium/High
   欄位的推論結果），並明確請使用者確認或修正——具體格式範例見下方兩份 template
   各自的欄位清單。

   嚴禁事項（即使在同一個持續執行的任務/單一 agent session 裡也適用，不因為
   執行環境是連續對話還是自動化 agent 而有例外）：輸出上述摘要後，絕對不要在
   同一次回覆中接著產出簡報大綱、指標樹、Mermaid 圖表，也絕對不要建立任何輸出
   資料夾或寫入任何檔案。這份摘要就是這一輪的終點，不是「先做這步、再自動接著
   做下一步」的其中一步。即使評估「使用者應該會確認」，也不能代替使用者做這個
   決定並自行 continue——必須真正等待使用者的下一則回覆。使用者可回覆「確認無誤，
   繼續」或直接給修正內容；也可回覆「不用再跟我確認，直接生成」跳過此檢查點——
   但只有在使用者明確給出上述其中一種回覆之後，才能繼續產出完整簡報大綱、講稿，
   以及第五階段的檔案/資料夾。
```
