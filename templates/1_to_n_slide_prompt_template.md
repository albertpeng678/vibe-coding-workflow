...
*   **第三步（草擬簡報大綱）**：依照「指標優先」故事線與「資料視覺化」佈局規則，草擬 7 頁簡報大綱。**特別注意：Slide 5 硬性要求為使用者互動流程圖；Slide 6 以體驗前後對比（Before/After）為主，純文字/卡片呈現，不需要生成 UI 圖片，系統整合細節僅需適度補充，不強制要求完整技術架構圖**。主產品名稱與北極星指標已於第一階段的收斂確認中與使用者對齊，後續各頁直接沿用確認後的結果。
...
*   **故事線稽核五綠燈標準**：
    ...
    3.  `[G]` **主產品定位確認與名詞定義**：檢查主產品名稱與產品北極星指標是否已在第一階段的「主產品定位收斂確認」中經使用者確認或修正——若簡報內容與確認結果不一致，視為不合格。簡報中出現的所有專有名詞，必須在第一次出現的頁面中進行白話文定義後才能使用。
...
*   **Few-Shot 範例**（主產品：Notion 平台，其 NSM = 每週活躍協作文件數；新增功能：AI 會議記錄整理）：
    ...
    *   [正例] *功能層成功*：每週生成的 AI 會議紀錄中，**在會後 48 小時內被除建立者外 ≥ 2 位團隊成員瀏覽** 的比例（反映團隊真正利用此功能進行協作與對齊；瀏覽本身已隱含「功能被採用」且「文件重新被團隊互動」兩個 FSM 必要條件，勾選這種高摩擦動作留給使用者自行決定要不要做，不該綁進指標本身）。
...
3.  **主產品定位收斂確認（正式產出簡報前的強制檢查點——這是一個硬性的對話輪次邊界，不是流程清單裡的一個段落）**：不論上一步是否觸發提問，在開始草擬簡報大綱與講稿之前，**必須先輸出一份精簡的「主產品定位摘要」，然後立即結束本次回覆**，彙整全部欄位目前確定/推論出的最終結果（含剛回答的 Low 欄位、以及 Medium/High 欄位的推論結果），並明確請使用者確認或修正，格式範例：

    ```text
    以下是我彙整的主產品定位，正式產出簡報前請確認或直接修正：
    - 主產品概述：[一段簡短描述，說明這個主產品實際是什麼、核心功能為何]
    - 主產品名稱與定位：[名稱與定位]
    - 主產品北極星指標：[指標定義]
    - 北極星指標基準值/目標值：[數字，若無則註明「尚未量測」]
    - 簡報受眾與使用場合：[受眾/場合]
    - 目前專案進度：[階段]
    請確認以上內容，或直接回覆修正——確認後我才會繼續產出完整簡報大綱。
    ```

    **嚴禁事項（即使在同一個持續執行的任務/單一 agent session 裡也適用，不因為執行環境是連續對話還是自動化 agent 而有例外）**：輸出上述摘要後，**絕對不要在同一次回覆中接著產出簡報大綱、指標樹、Mermaid 圖表，也絕對不要建立任何輸出資料夾或寫入任何檔案**。這份摘要就是這一輪的終點，不是「先做這步、再自動接著做下一步」的其中一步。即使你評估「使用者應該會確認」，也不能代替使用者做這個決定並自行 continue——必須真正等待使用者的下一則回覆。使用者可回覆「確認無誤，繼續」或直接給修正內容；也可回覆「不用再跟我確認，直接生成」跳過此檢查點——但只有在使用者明確給出上述其中一種回覆之後，才能繼續產出完整簡報大綱、講稿，以及第五階段的檔案/資料夾。
...
*   **Slide 1: 項目現狀 (Current Status)**
    *   *大標題*：我們目前以產品北極星指標 [主產品產品北極星，已於第一階段確認] 衡量平台價值。
...
*   **Slide 6: 解方體驗對比 (Solution Experience Before/After)**
    *   *大標題*：這個功能上線後，使用者的核心體驗會有什麼質變。
    *   *內容*：**以「前後對比」呈現體驗本身的質變，不是重複 Slide 5 的操作步驟**——用左右對稱卡片描述「現在（Before）：使用者要花時間/心力做的手動、麻煩的事」對比「上線後（After）：一鍵、自動、無感的體驗」，純文字/卡片呈現即可，不需要生成任何 UI 截圖或線框圖。系統整合方式僅需一句話摘要帶過（例如與既有系統的串接重點），不強制要求完整系統架構圖。
*   **Slide 7: 驗證計畫 (Validation Plan)**
    *   *大標題*：驗證計畫：以指標樹把關成效，防堵未能反映使用者實際感受價值的指標。
    *   *內容*：
        *   第一層 (Top)：產品北極星指標 (NSM，已於第一階段確認) ＋ **設計與採用理由**。
...
## Slide 1: [第一頁標題 - 項目現狀]
*   主產品：[主產品名稱，已於第一階段與使用者確認]
*   產品北極星指標：[指標與數值，已於第一階段與使用者確認]
...
## Slide 6: [第六頁標題 - 解方體驗對比]
*   體驗前後對比（主要內容，左右對稱卡片，給 Gamma 排版文字用）：
    *   Before（現況）：[描述使用者現在要花時間/心力做的手動、麻煩的事]
    *   After（上線後）：[描述功能上線後，變成一鍵、自動、無感的體驗]
*   系統整合摘要（次要補充，一句話帶過即可）：[既有主產品架構] ➔ [AI 核心運算層] ➔ [新功能交互介面]
*   （可選）對應的 Mermaid 圖表（見「第四階段：流程圖策略」的「Z 字型技術管線圖」範本）——僅在需要向技術背景主管佐證整合可行性時使用，非本頁必要元素；**使用者需自行用 Mermaid 渲染工具轉成圖片後插入本頁**。
*   內容描述（供演講稿使用）：
    *   體驗質變重點：[描述 Before/After 對比的核心差異，嚴禁使用無結構文字]
    *   既有系統整合點（次要）：[描述與主產品系統的串接細節，一句話摘要即可]

## Slide 7: [第七頁標題 - 驗證計畫]
*   驗證指標樹：
    *   產品北極星指標 (NSM，已於第一階段確認)：[定義] ➔ 採用理由：[必須硬性說明]
    *   功能層成功指標 (FSM)：[定義] ➔ 採用理由：[必須硬性說明如何拉動產品北極星指標，避開高摩擦留言/點讚動作，採用低摩擦團隊瀏覽瀏覽代理行為]
    *   量能指標 (Volume)：[定義] ➔ 採用理由：[必須硬性說明如何度量漏斗前端曝光 reach]
    *   護欄指標 (Guardrail)：[定義，若用 NPS 必須以 `X.X/5.0` 滿意度評分格式表示（禁止原始 NPS 分數），並配對一個行為日誌指標] ➔ 採用理由：[必須硬性說明如何防範指標存在被少數重度用戶或短期效應灌歪的風險]
...
| Slide 6 | [標題] | PASS (已驗證無指標偏差) | PASS (已確認以體驗前後對比為主、系統整合僅為補充) | PASS |
