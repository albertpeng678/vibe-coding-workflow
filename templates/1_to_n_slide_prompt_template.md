...
*   **第二步（信心分級澄清 - Human-in-the-Loop）**：依下方「第一階段：主產品定位與指標對齊（信心分級澄清機制）」執行信心自評；Low 信心時停止並向使用者提問，Medium/High 信心則推論並寫出依據，定調**產品北極星指標（價值總量）**。
*   **第三步（草擬簡報大綱）**：依照「指標優先」故事線與「資料視覺化」佈局規則，草擬 7 頁簡報大綱。**特別注意：Slide 5 與 Slide 6 均硬性要求為資料視覺化架構圖/流程圖**。且強制在主產品與指標所有提及處標註 `[請在此處更新您正確的主產品名稱及指標]`。
...
*   **指標稽核三綠燈標準**：
    1.  `[G]` **價值度量**：產品北極星指標 (NSM) 必須衡量「使用者感受到的價值」(需求面)，嚴禁採用「系統做了什麼」的供給面指標，且該指標必須在當前條件下可量測。且**每一個指標都必須附帶明確的「設計與說服理由/採用理由」說明**。一律禁止使用「大盤指標」等不專業敘述，統一使用「產品北極星指標」。
    2.  `[G]` **1-to-N 結構合規**：**既有功能更新提案 (1 to N) 必須包含完整 3 層指標樹** (產品北極星指標 ➔ 功能層成功指標 ➔ 量能指標 ｜ 護欄指標)。功能層成功指標 (FSM) 必須明確定義，且其能合理拉動產品北極星指標。
...
*   **故事線稽核五綠燈標準**：
    1.  `[G]` **故事線通順度**：7 頁故事線必須完美契合「指標優先」的主線（現狀 ➔ 瓶頸 ➔ 用戶故事 ➔ 成功定義 ➔ 使用者互動流程 ➔ 解方原型與系統架構 ➔ 驗證計畫），前後因果邏輯必須緊密相扣，轉換流暢自然。
    2.  `[G]` **使用者互動圖硬性要求**：**Slide 5 必須是「使用者互動流程圖」（而非後台技術架構圖）**。必須以圖形化卡片與連接關係（例如：用戶輸入端 ➔ 核心交互步驟 ➔ 系統反饋 ➔ 輸出端）來描述，且此流程圖與架構圖必須嚴格基於 `design-system-portfolio-site_6640.md` 的視覺設計規範，嚴禁使用純文字條列來敷衍互動流程。
    3.  `[G]` **警告標籤與定義**：檢查是否在所有主產品名稱與產品北極星指標旁均緊鄰標註 `[請在此處更新您正確的主產品名稱及指標]`。簡報中出現的所有專有名詞，必須在第一次出現的頁面中進行白話文定義後才能使用。
...
*   **Few-Shot 範例**：
    ...
    *   [正例] *用戶感受價值*：每週成功 **匯出或分享** 的 AI PRD 總數量（匯出代表內容已通過人工檢視且將進入下一步工作流）。
    *   [反例] *落後/財務*：付費訂閱用戶數（無法指導產品改進的結果指標）。
    *   [正例] *用戶感受價值*：每週有進行內容分享或協作的活躍 workspace 數量。
...
*   **Slide 1: 項目現狀 (Current Status)**
    *   *大標題*：我們目前以產品北極星指標 [主產品產品北極星] 衡量平台價值 `[請在此處更新您正確的主產品名稱及指標]`。
...
*   **Slide 7: 驗證計畫 (Validation Plan)**
    *   *大標題*：驗證計畫：以指標樹把關成效，防堵未能反映使用者實際感受價值的指標。
    *   *內容*：
        *   第一層 (Top)：產品北極星指標 (NSM) ＋ **設計與採用理由** `[請在此處更新您正確的主產品名稱及指標]`。
...
## Slide 1: [第一頁標題 - 項目現狀]
*   主產品：[主產品名稱] [請在此處更新您正確的主產品名稱及指標]
*   產品北極星指標：[指標與數值] [請在此處更新您正確的主產品名稱及指標]
*   指標採用與說服理由：[必須硬性說明為什麼代表核心價值的兌現，並解釋為什麼排除註冊數或營收等未能反映使用者實際感受價值的供給面/落後指標]
*   主管視角：[說明]
...
## Slide 7: [第七頁標題 - 驗證計畫]
*   驗證指標樹：
    *   產品北極星指標 (NSM) [請在此處更新您正確的主產品名稱及指標]：[定義] ➔ 採用理由：[必須硬性說明]
    *   功能層成功指標 (FSM)：[定義] ➔ 採用理由：[必須硬性說明如何拉動產品北極星指標，避開高摩擦留言/點讚動作，採用低摩擦團隊瀏覽瀏覽代理行為]
    *   量能指標 (Volume)：[定義] ➔ 採用理由：[必須硬性說明如何度量漏斗前端曝光 reach]
    *   護欄指標 (Guardrail)：[定義，若用 NPS 必須配對一個行為日誌指標] ➔ 採用理由：[必須硬性說明如何防範指標存在被少數重度用戶或短期效應灌歪的風險]
...
| :--- | :--- | :--- | :--- | :--- |
| Slide 1 | [標題] | PASS (已對齊產品北極星指標，非自嗨且附帶採用理由) | PASS (通過理由) | PASS |
| Slide 2 | [標題] | PASS (通過理由) | PASS (通過理由) | PASS |
| Slide 3 | [標題] | PASS (通過理由) | PASS (通過理由) | PASS |
| Slide 4 | [標題] | PASS (通過理由) | PASS (通過理由) | PASS |
| Slide 5 | [標題] | PASS (已驗證無指標偏差) | PASS (已確認為使用者互動流程圖) | PASS |
| Slide 6 | [標題] | PASS (已驗證無指標偏差) | PASS (已確認為系統整合架構圖) | PASS |
| Slide 7 | [標題] | PASS (包含完整 3 層樹，且 NPS 護欄有進行雙重日誌配對，各指標皆附帶採用理由，統一使用「產品北極星指標」專業敘述) | PASS (警告標籤已置入) | PASS |
