...
*   **第三步（草擬簡報大綱）**：依照「指標優先」故事線與「資料視覺化」佈局規則，草擬 7 頁簡報大綱。**特別注意：Slide 5 硬性要求為使用者互動流程圖；Slide 6 以解方原型介面展示為主，系統整合細節僅需適度補充，不強制要求完整技術架構圖**。主產品名稱與北極星指標已於第一階段的收斂確認中與使用者對齊，後續各頁直接沿用確認後的結果。
...
*   **Slide 6: 解方原型展示 (Solution Prototype Showcase)**
    *   *大標題*：新功能原型介面與核心操作體驗設計。
    *   *內容*：**主要聚焦解方原型介面**（UI 截圖/線框示意、核心操作路徑），讓主管一眼看懂使用者如何操作、獲得什麼價值；系統整合方式僅需一句話摘要帶過（例如與既有系統的串接重點），不強制要求完整系統架構圖，避免過度技術化分散主管對解決方案本身的注意力。
...
## Slide 6: [第六頁標題 - 解方原型展示]
*   解方原型介面重點（主要內容，給 Gamma 排版文字用）：[描述 UI 原型的核心畫面/操作路徑，讓主管一眼看懂使用者價值]
...
*   內容描述（供演講稿使用）：
    *   原型介面重點：[描述 UI 與核心操作路徑，嚴禁使用無結構文字]
...
| Slide 6 | [標題] | PASS (已驗證無指標偏差) | PASS (已確認以解方原型介面為主、系統整合僅為補充) | PASS |
