...
*   **故事線稽核三綠燈標準 (🟢🟢🟢)**：
    ...
    2.  `🟢` **架構圖硬性要求**：**Slide 5 必須是「使用者互動與功能機制流程圖」，Slide 6 必須是「原型設計與系統整合架構圖」**。這兩頁只要在 PPTX 中有採用架構/流向圖，就必須以圖形化卡片與連接關係（例如：輸入端 ➔ AI 核心處理 ➔ 雙重驗證 ➔ 輸出端）來描述，且此流程圖與架構圖必須嚴格基於 `design-system-portfolio-site_6640.md` 的視覺設計規範，嚴禁使用純文字條列來敷衍。
...
指標稽核官與生成引擎必須嚴格使用以下定義、Few-Shot 範例與設計理由規範來處理簡報中的指標：
...
*   **設計與稽核原則**：
    *   **專業敘述要求**：**嚴禁使用「大盤指標」這種不專業的敘述方式**，統一使用「產品北極星指標」或「產品北極星指標（價值總量）」。
    *   **價值實現代理**：用戶在系統點擊「生成」不代表感受到價值；必須是「匯出檔案」、「複製內容」或「分享給協作者」等代表內容已被人工檢視且即將進入下一步工作流的動作。
    *   **避開百分比陷阱**：產品北極星指標一律使用**「絕對計數」**（例如：每週成功匯出的 PRD 數量），以衡量業務規模與實質用戶價值的總和。避免使用率或百分比。
    *   **硬性採用理由**：在簡報中必須詳細說明為什麼該指標代表核心價值的兌現，並解釋為什麼排除註冊數或營收等自嗨/落後指標。
*   **Few-Shot 範例**：
    *   ❌ *自嗨/供給面*：每週 AI 生成 PRD 的總次數（用戶可能生成後立即刪除，未感受到價值）。
...
*   **Few-Shot 範例**（主產品：Notion 平台，其大盤 NSM = 每週活躍協作文件數；新增功能：AI 會議記錄整理）：
    *   ❌ *自嗨/虛胖*：AI 會議記錄功能點擊率（僅反映好奇心，不代表真實轉化）。
...
*   **定義**：用於監控品質、安全、用戶疲勞度或系統副作用的防禦指標，用以**排除行為偽裝（防範成功虛胖）**。
...
*   **設計與稽核原則**：
    ...
    *   **硬性採用理由**：必須在簡報中詳細說明此護欄指標是如何防止「成功虛胖」的。
*   **Few-Shot 範例**：
...
2.  **定位產品北極星 (模擬 5 隻 Agent 共識會議)**：若 spec 中未寫明主產品，請自動啟動 5 隻虛擬 Agent 進行共識探討定調。同時，進行同類產品的 Web Search，以徹底了解主產品定位，並設定主產品的**產品北極星指標（價值總量）**。
3.  **提醒與警告標記**：在最終輸出的簡報大綱與講稿中，針對推論出來的主產品名稱與產品北極星指標，必須**強制在旁加上顯著的警告標記**，提醒使用者：
