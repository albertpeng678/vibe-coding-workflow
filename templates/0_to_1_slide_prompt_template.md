...
*   **第二步（並行搜尋調研 - Fan-out）**：針對痛點自動展開 10 個搜尋查詢，模擬 10 隻子 Agent 並行調研真實世界數據、權威來源（如 McKinsey, Asana, Bloomberg）與精確網址。
...
*   **Emoji 防護欄**：檢查大綱與 Gamma 指令中是否含有任何表情符號（如 ↗, ✅, ⚠️ - ⚠️除外，僅限於主產品警告標籤）？如果有，必須全部移除。
*   **指標結構防護欄**：檢查 Slide 6 的驗證計畫是否**完全沒有**出現「功能層成功指標 (FSM)」？如果有，必須將其刪除，只保留北極星、量能與 5 次編輯護欄。
*   **呼吸感與密度防護欄**：檢查每張幻燈片的項目是否 ≦ 3 條？文字是否過於擁擠？如果有，必須進行精簡（文字密度為 Brief）。
*   **佈局多樣性防護欄**：檢查是否有連續兩頁使用相同的佈局模式（如連續兩個 Bento Grid）？如果有，必須調整以維持節奏感。
