...
*   **指標稽核三綠燈標準**（以下三項標準皆隱含一項共同前提：**每個指標都必須附帶具體可執行的量測方法**——如 log 欄位、SQL 查詢邏輯、LLM-as-a-Judge 評分機制等，不能只憑空宣稱「可量測」，寫不出具體方法者視為不合格）：
    ...
    3.  `[G]` **防衛健康與 NPS 配對**：量能指標必須為全域加總（防範 session 偏差）；護欄指標必須是可量測的比率或指標，嚴禁使用絕對值限制 (如 = 0) 或已被技術解決的偽問題。**若採用 NPS 作為護欄，必須硬性多配對一個「行為日誌指標 (Behavioral Logs Metric)」**（如刪除率、修改輪數、還原率）作為雙重防守，且**呈現數字必須是 `X.X/5.0` 滿意度評分格式，禁止使用原始 NPS 分數（如 30、60 等無上下文數字）**。
...
*   **設計與稽核原則**：
    ...
    *   **價值實現代理**：
        **第一步（必做，且必須先於任何範例）**：請先分析 `spec.md`/`plan.md`，推導出「使用者在什麼情境下，代表他真正拿到了這份簡報/功能的價值」的具體動作。在完成這一步、寫下你自己推導出的答案之前，不要參考下方範例。
        **第二步（範例僅供核對原則涵蓋度，非選項清單）**：以下三個範例示範的是「動作發生在使用者真正檢視/使用產出物的那一刻」這個共通原則，並非窮舉選單：
        - 「匯出檔案」（示範原則：內容離開系統、進入下一步工作流）
        - 「複製內容」（示範原則：使用者主動取用內容、代表已完成檢視）
        - 「分享給協作者」（示範原則：內容被推向他人、代表已被判定值得傳遞）
        你在第一步推導出的動作，很可能與上述三例完全不同——這是正常的，只要符合「用戶在系統點擊『生成』不代表感受到價值，必須是內容已通過人工檢視、即將進入下一步工作流」這個共通原則即可。**若你的最終輸出與範例文字完全相同、卻沒有引用 spec.md/plan.md 中的具體依據，視為分析未完成，須重新推導。**
...
*   **設計與稽核原則**：
    *   **NPS 強制配對規則**：如果護欄指標採用了 **NPS (淨推薦值)** 等主觀問卷指標，**AI 必須硬性在旁邊多配對一個「行為日誌指標 (Behavioral Logs Metric)」**（例如：生成後 5 分鐘內的刪除率、人工作業二次修改輪數中位數、或還原率），作為雙重防守。**呈現格式**：一律以 `X.X/5.0` 的滿意度評分表示（例如「4.2/5.0」），不要使用原始 NPS 分數量表（如 -100~100 或 30、60 這類無上下文的數字），避免讀者需要額外理解 NPS 計算方法才能判讀。
...
*   **Few-Shot 範例**：
    ...
    *   [正例] *可優化比率*：每週 AI 產出內容的**人工作業修正率**（如修改輪數超過 5 次的比例 ≦ 10%；**量測方法**：於每次修改事件寫入 `revision_round` 計數欄位，匯出前用 SQL 統計 `revision_round > 5` 的匯出數 ÷ 全部匯出數）。
...
2.  **依信心分級決定動作**：
    *   任一欄位為 **Low** → 一次性列出所有 Low 欄位的澄清問題（最多 5 題），
        每題附上「AI 推論候選值，請確認或修正」；問題前先用一句話說明為什麼問這個。
        等待使用者回覆。使用者可回覆「不用問我，直接用你的推論」跳過此步驟。
    *   全部為 **Medium/High** → 不觸發提問，直接進入下一步；仍需完成下方第 3 步的收斂確認。
3.  **主產品定位收斂確認（正式產出簡報前的強制檢查點）**：不論上一步是否觸發提問，在開始草擬簡報大綱與講稿之前，**必須先輸出一份精簡的「產品定位摘要」**，彙整全部欄位目前確定/推論出的最終結果，並明確請使用者確認或修正，格式範例：

    ```text
    以下是我彙整的產品定位，正式產出簡報前請確認或直接修正：
    - 主產品概述：[一段簡短描述，說明這個全新產品實際是什麼、核心功能為何]
    - 核心價值主張：[一句話定位與核心價值主張]
    - 未來北極星指標：[定義]
    - 簡報受眾與使用場合：[受眾/場合]
    - 目前產品進度：[階段]
    請確認以上內容，或直接回覆修正——確認後我才會繼續產出完整簡報大綱。
    ```

    使用者可回覆「確認無誤，繼續」或直接給修正內容；也可回覆「不用再跟我確認，直接生成」跳過此檢查點。**只有在使用者明確回覆之後，才繼續產出完整簡報大綱與講稿。**
4.  **痛點識別**：分析 `spec.md` 中產品欲解決的核心痛點（例如：人工填報耗時、錯誤率高、合規成本等）。
5.  **搜尋策略 (模擬 10 隻平行 Agent)**：自動將該痛點展開為 10 個獨立且具體的搜尋指令，去尋找國內外的權威統計數據與案例。
6.  **數據融入**：你必須在簡報的 **Slide 1 (問題探索)** 放入至少 **2 個真實的量化數據**（包含數據來源與網址連結），以此作為提案說服力的核心支柱。
...
*   **Slide 4: 使用者互動與流程圖 (User Interaction & Flowchart)**
    *   *大標題*：解方核心互動流程設計。
    *   *內容*：**硬性要求規劃使用者互動與流程圖**（以 Mermaid 流程圖呈現，見「第四階段：流程圖策略」，不需要另外產出 UI 介面圖片）。描述用戶如何輸入、系統如何引導、AI 階段處理（如 Extractor & Verifier）如何將資訊反饋給用戶，以及最終的輸出互動。
...
*   驗證指標樹 (無功能層指標)：
    ...
    *   護欄指標 (Guardrail)：[定義，若用 NPS 必須以 `X.X/5.0` 滿意度評分格式表示（禁止原始 NPS 分數），並配對一個行為日誌指標] ➔ 採用理由：[說明如何防範指標存在被少數重度用戶或短期效應灌歪的風險]
